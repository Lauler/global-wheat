...
df_train_img <- tibble(image_id = dir("Yet-Another-EfficientDet-Pytorch/datasets/wheat/train") %>% stringr::str_remove(".jpg")) %>%
  left_join(df_all_img)
df_valid_img <- tibble(image_id = dir("Yet-Another-EfficientDet-Pytorch/datasets/wheat/validation") %>% stringr::str_remove(".jpg")) %>%
  left_join(df_all_img)
...
annotation_list <- function(df, instances){
  ...
  for (i in 1:nrow(df)){
    img_list[[i]]$id <- df$id.y[i] # bbox ids
    img_list[[i]]$image_id <- df$id.x[i] # image id
